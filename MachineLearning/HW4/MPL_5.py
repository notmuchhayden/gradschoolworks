...
# (5) (40점) 1.(2)~1.(3)의 과정을 은닉 뉴런이 3개인 경우에 대해서도 수행하시오. 이때 반복횟
# 수가 1,000회가되기 전 오류율이 0.05미만으로 낮아지는 경우(올바른 학습 예)와 그렇지
# 않은 경우(불완전 학습 예)의 두 가지 경우(아래 그림 참조)를 모두 관찰하고 각각의 결정
# 경계를 표시한 후 그 결과를 비교하시오.
# (※ 랜덤하게 발생하므로 여러 번 수행하는 것이 필요할 수 있음)
...
# 다층 퍼셉트론 구성
N = X.shape[0]                              # 데이터의 수
INP, HID, OUT = 2, 3, 1                     # 입력 뉴런 수, 은닉 뉴런 수, 출력 뉴런 수
