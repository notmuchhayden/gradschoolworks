...
ChaCha20::ChaCha20(const Key& key, const Nonce& nonce) {
    ...
    initialState_[1] = load32(Sigma.data() + 4);
    initialState_[2] = load32(Sigma.data() + 8);
    initialState_[3] = load32(Sigma.data() + 12);

    // 둘째와 셋째 행(4~11)에 256비트 키를 연속으로 배치한다.
    for (std::size_t i = 0; i < 8; ++i) {
        initialState_[4 + i] = load32(key.data() + i * 4);
    }

    // 원 논문의 64비트 카운터/64비트 논스 형식을 사용한다.
    // 마지막 행의 12~13번은 카운터, 14~15번은 논스이다.
    initialState_[14] = load32(nonce.data());
    initialState_[15] = load32(nonce.data() + 4);
}

ChaCha20::Block ChaCha20::generateBlock(std::uint64_t blockCounter) const {
    // 키와 논스가 들어 있는 공통 상태를 복사하고 현재 카운터를 설정한다.
    auto state = initialState_;
    state[12] = static_cast<std::uint32_t>(blockCounter);
    state[13] = static_cast<std::uint32_t>(blockCounter >> 32);
