...
| `delay_probe_count` | Qdrant 갱신 지연 시간 동안 반복 검색을 수행한 횟수 |
| `delay_observed_seconds` | 반복 검색으로 실제 관찰한 stale 노출 시간 |
| `delay_old_meaning_retrieval_total` | 지연 시간 동안 old meaning query로 stale 문서가 검색된 누적 횟수 |
| `delay_new_meaning_retrieval_total` | 지연 시간 동안 new meaning query로 수정 문서가 검색된 누적 횟수 |
| `qdrant_vector_update_latency_ms` | Qdrant vector와 payload를 실제로 갱신하는 데 걸린 시간 |
| `update_latency_ms` | 문서 수정, 지연 중 측정, 벡터 갱신을 포함한 end-to-end 처리 시간 |

stale vector ratio 계산:

```text
stale vector ratio = 최신 원문과 embedding_version이 불일치한 문서 수 / 수정 대상 문서 수
```
...
- 지연 시간이 길어질수록 갱신 전후의 상태값 자체는 같을 수 있지만, `delay_probe_count`와 `delay_observed_seconds`가 증가하여 stale 상태가 더 오래 노출되었음을 보여준다.
- `delay_old_meaning_retrieval_total`이 높고 `delay_new_meaning_retrieval_total`이 낮으면 지연 시간 동안 검색이 계속 예전 의미를 기준으로 작동했다는 뜻이다.
...
| 구조 | 지연 조건 | stale before | stale after | old before | old after | new before | new after | probe docs | delay probes | observed sec | old during delay | new during delay | qdrant update ms | end-to-end ms |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| Qdrant 분리형 | 0초 |  |  |  |  |  |  |  |  |  |  |  |  |  |
| Qdrant 분리형 | 5초 |  |  |  |  |  |  |  |  |  |  |  |  |  |
| Qdrant 분리형 | 30초 |  |  |  |  |  |  |  |  |  |  |  |  |  |
